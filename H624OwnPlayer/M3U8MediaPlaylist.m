...
- (instancetype)initWithContentOfURL:(NSURL *)URL type:(M3U8MediaPlaylistType)type andFormat:(M3U8ExtXStreamInf*) format error:(NSError **)error {
    if( [self initWithContentOfURL:URL type:type error:error])
    {
        _format = format;
        return self;
...
- (void)parseMediaPlaylist {
    ...
    while (NSNotFound != segmentRange.location) {
        ...
        // self.baseURL
        //M3U8SegmentInfo *segment = [[M3U8SegmentInfo alloc] initWithDictionary:params andStartTime:startTime withNumber:++segmentNumber playListUr:self.baseURL];
        M3U8SegmentInfo *segment = [[M3U8SegmentInfo alloc] initWithDictionary:params
                                                            andStartTime:startTime
                                                            withNumber:++segmentNumber
                                                            mediaPlayList:[self baseURL]];
        
        // M3U8SegmentInfo *segment = [[M3U8SegmentInfo alloc] initWithDictionary:params andStartTime:startTime];
        if (segment) {
