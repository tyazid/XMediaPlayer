...
- (void)parseMasterPlaylist {
    ...
    while (NSNotFound != crRange.location) {
        ...
        if([line length] &&
           [line characterAtIndex:[line length] -1] == 0xd)
        ...
        // http://hls.ted.com/talks/769/video/600k.m3u8?sponsor=Ripple
         ...
         //  NSLog(@"\t\t ATTRs : %@",attribute_list);
            ...
            if(nextLine &&
               [nextLine length] &&
                 [nextLine characterAtIndex:[nextLine length] -1] == 0xd)
                  nextLine = [nextLine substringToIndex:[nextLine length] - 1];
            ...
            remainingPart = [remainingPart substringFromIndex:crRange.location +1];
            crRange = [remainingPart rangeOfString:@"\n"];
           // NSLog(@"\t\t remainingPart : %@",remainingPart);
          
////// FORMAT HAS TO BE CREATED HERE
     ...
     [self.xStreamList sortByBandwidthInOrder:_Used_NSComparisonResult_To_Sort_StreamList_];
}
