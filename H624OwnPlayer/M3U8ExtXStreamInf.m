...
- (NSString *)m3u8URL {
   ...
   // NSLog(@">>>>>>>>> M3IU8ExtXstreamInfo [self baseURL]:: %@ URL : %@ self.URI:%@",[self baseURL],baseURL,self.URI);
    NSString* adr  = [NSString stringWithFormat:@"%@%@",[self baseURL],self.URI] ;
    adr  = [adr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];

    //NSLog(@">>>>>>>>> M3IU8ExtXstreamInfo  --> finally(%@) -> : %@ ",adr, [NSURL URLWithString: adr ] );
...
- (NSString *)description {
    return [NSString stringWithFormat:@"centent:%@ -- PL URL: %@",  [NSString stringWithString:self.dictionary.description],[self m3u8URL]];
}
