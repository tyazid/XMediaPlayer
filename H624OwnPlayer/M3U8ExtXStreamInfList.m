...
/* reat NSUIntegerMax if not found*/
- (NSUInteger)indexOf:(M3U8ExtXStreamInf*) inf{
    @try {
        return [self.m3u8InfoList indexOfObject:inf];
...
- (M3U8ExtXStreamInf *)xStreamInfAtIndex:(NSUInteger)index {
    if (index >= self.count) {
        @throw   [NSException
                  exceptionWithName:NSRangeException reason:[NSString stringWithFormat:@"index:%lu, range:0-%lu",index,self.count-1]
                  userInfo:nil];
        return nil;
    }
